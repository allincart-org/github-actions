# Reusable GitHub Actions and Workflows for Allincart extensions
...
Installs Allincart 6, PHPStan and the extension, then runs PHPStan
...
Builds the extension zip and validates the zip using allincart-cli
...
Upload the extension with the given Allincart account credentials into the account. It is recommended to use the `workflow_dispatch` event, so you have to manually trigger this from the Actions Tab.
...
subject: repo:allincart-org/allincart:ref:refs/heads/main
...
This policy only allows the `main` ref with the `downstream.yml` workflow of the `allincart-org/allincart` repository to get a token with the `actions:write` permissions.
